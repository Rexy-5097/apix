...
#
# APIx UPSTREAM PATCH UP-001 (declared, not silent) — see docs/agentos/UPSTREAM_PATCHES.md
# Upstream raptors-way@2cf150f hardcodes an author-specific absolute macOS path here:
#     REPO_ROOT = "/Users/soumyadebtripathy/WorkFlow/agentos-template"
# That makes validate_agentos.py resolve every path outside the repository on any
# other machine, so it reports 0/100 FAIL unconditionally. AGENTOS.md section 12
# requires a 100/100 PASS before initialization may be declared complete, so the
# validator is unusable as shipped and AgentOS cannot be verified without this fix.
#
# The replacement uses the __file__-derived idiom already used by the sibling
# script tools/scripts/bootstrap_project.py, so the two agree on the repo root.
# tools/scripts/<file> -> dirname x3 -> repository root.
...
class Validator:
    ...
    def validate_artifacts(self):
        ...
        # APIx UPSTREAM PATCH UP-004 (declared, not silent) — see docs/agentos/UPSTREAM_PATCHES.md
        # Upstream walks the ENTIRE repository and treats every Markdown file
        # carrying YAML frontmatter as an AgentOS artifact requiring all 12
        # metadata keys. YAML frontmatter is the universal convention for agent
        # skills, static-site content and note vaults, so vendoring any such
        # content floors this category at 0/100 and permanently masks a genuine
        # malformed APIx artifact. APIx vendors 25 Agent Skills under .claude/,
        # each with a 2-key frontmatter block: 25 x 12 = 300 phantom violations.
        # Scoping the walk to directories AgentOS actually owns restores the
        # signal. Same idiom as the existing .git / node_modules exclusion.
        # ``.github`` is excluded deliberately. GitHub issue templates carry
        # GitHub's own frontmatter schema (name/about/title/labels), which is not
        # the AgentOS artifact schema and should never be measured against it.
        # Upstream excluded them only by accident: its check is the substring
        # ``".git" in root_dir``, and ".github" contains ".git". Matching whole
        # path segments instead removes that accident, so the exclusion has to be
        # stated on purpose to keep upstream's behaviour for the right reason.
        vendored = (
            ".git",
            ".github",
            "node_modules",
