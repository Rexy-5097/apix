...
| **All 35 accepted observations pass the corresponding checks with zero false rejections** | `data/panel.json` → `observations` | control arm of the same replay | `test_no_accepted_observation_is_falsely_rejected` | **ESTABLISHED** | A rejection rule that also rejected the accepted panel would be worthless; this is the check that rules that out |
